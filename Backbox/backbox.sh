...
if [ "$first" != 1 ];then
        if [ ! -f $tarball ]; then
                ...
                *)
                ...
                wget "https://github.com/EXALAB/Anlinux-Resources/raw/master/Rootfs/BackBox/${archurl}/${ubuntu}-rootfs-${archurl}.tar.xz" -O $tarball
        fi
        cur=`pwd`
        mkdir -p "$folder"
        cd "$folder"
...
bash $linux
   clear
   echo ""
   echo "You can now start Backbox with 'backbox' script next time"
   echo ""
#rm backbox.sh
