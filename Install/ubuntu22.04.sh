...
if [ "$first" != 1 ];then
        if [ ! -f $tarball ]; then
                ...
                *)
                ...
                wget "https://partner-images.canonical.com/oci/jammy/current/ubuntu-lunar-oci-${archurl}-root.tar.gz" -O $tarball
        fi
        cur=`pwd`
        mkdir -p "$folder"
        cd "$folder"
...
deb http://ports.ubuntu.com/ubuntu-ports jammy-backports main restricted universe multiverse" > ~/"$folder"/etc/apt/sources.list
   clear
   echo " "
